...
| 경로 | 내용 |
|---|---|
| `/` | **테스트 모음 허브 (4주차 신규)** — 햄버거 메뉴(☰) → 카테고리 드로어(전체/성격유형/감정심리), 소개 문단 2개, 카테고리 탭, 테스트 카드 그리드(정사각 썸네일+제목+설명+소요시간+카테고리 pill+NEW 뱃지). 참여자 수 등 가짜 숫자는 넣지 않음. MBTI 선택 폼은 더 이상 여기 없음 — `/mbti`로 이동됨 |
...
**① 성인 애착유형 테스트 (`src/app/tests/attachment/`, 초안)**
...
- **문항 카피 검토 대기**: 테토-에겐 q1~q12(초안), 성인 애착유형 10문항(전체 초안) — 확정 필요
- 로고(시바견) 파일 전달받으면 파비콘/헤더 삽입 진행 — 그 전까지 헤더는 텍스트 로고만
