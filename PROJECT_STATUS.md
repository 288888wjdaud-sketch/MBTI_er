...
| `/tests/mental-age` | **정신연령 테스트** (8주차 신규) 8문항, 축 없이 옵션별 나이 점수 평균 (초안) |
| `/tests/balance-game` | **밸런스게임** (8주차 신규) 12쌍 A/B 양자택일, 선택 요약 리포트 중심 (초안) |
| `/tests/slang-trend` | **신조어 트렌드 지수** (8주차 신규) 정답형 3지선다 10문항, 맞춘 개수로 채점 (초안, 시의성 주의) |
| `/tests/couple-sync` | **커플 싱크로율 테스트** (8주차 신규) 15문항, URL 인코딩으로 두 사람 답변 비교(DB 없음) (초안) |
| `/tests/couple-sync/[code]` | 커플 싱크로율 2단계(상대방) 진입점 — `ƒ` 동적 렌더링, `[code]`는 base64url 인코딩된 A의 답변 |
...
- `SiteShell`(client, 6주차 신규, `layout.js`에서 `{children}` 감쌈) — `usePathname()`으로 `/`·`/tests/*`(배경
  패턴만) vs 그 외(기본)를 구분해서 적용. 6주차엔 `/mbti/*`에 좌우 `SideRail`을 붙였었으나 7주차에 앱 프레임
  전환으로 레일 분기를 제거함(아래 7주차 항목 참고) — 지금은 패턴 전용/기본 2분기만 남음.
- `SideRail`(server, 6주차 신규) — **7주차부터 미사용**(`SiteShell.js`에서 import 제거). 파일은 삭제하지
  않고 남겨둠 — 애드센스 승인 후 프레임 바깥 여백에 광고 자리로 재활용할 가능성 있음(Spec.md 2.5.10).
- `RelatedTests`(server, 6.1주차 신규) — 퀴즈 결과 화면 본문 안에 다른 테스트 1~2개를 작게 추천(사이드레일과
  별개, 모바일에서도 항상 보임). 5개 퀴즈 결과 화면에 공용으로 사용. 7주차 앱 프레임 전환 이후에도 그대로 유지.
...
- 모바일 퍼스트, 7주차부터 전 페이지가 `.appFrame`(max-width 480px, 중앙 정렬)에 헤더~본문~푸터가 통째로
  들어가는 "앱 프레임" 레이아웃 — 넓은 화면에서도 모바일 앱처럼 좁은 프레임 하나로 보임(poomang 레퍼런스
  참고, 위 7주차 항목 참고). 6주차의 좌우 `SideRail` 방식은 폐기.
...
## 6.1주차 — 사이드레일 버그 수정 + 접근성 보완 (2026-07-11, Spec.md 2.5.9)

사장님이 로컬에서 직접 확인하다가 홈 화면에서 같은 테스트 카드가 좌측 레일/가운데 그리드/우측 레일에
3중으로 중복 노출되는 버그를 발견(스크린샷 확인). 원래 의도(다른 페이지에서 다른 테스트로 유도)와
다르게 홈에까지 레일이 붙어서 생긴 기획 실수였음.

**① 사이드레일 수정**
- `SiteShell.js`의 라우팅 로직을 "패턴 전용(`/`, `/tests/*`) vs 레일(`/mbti/*`) vs 기본(그 외)" 3분기로
  재정리. 홈은 이제 퀴즈 페이지와 동일하게 레일 없이 배경 패턴만 적용됨 — 중복 해소.
- `/mbti/[type]`, `/mbti/match/[pair]`는 경로가 `/mbti/`로 시작하지만 `/mbti`(허브, 트레일링 슬래시 없음)는
  안 걸리는 걸 이용해서 별도 페이지 수정 없이 접두어 매칭만으로 정확히 구분함.
- 브라우저로 홈(레일 없음, 배경 패턴만), `/mbti/estj`(레일 노출), `/mbti/match/estj-infp`(레일 노출) 전부
  직접 스크린샷 확인. 콘솔 에러 없음.

**② 접근성 보완**
- `layout.js`의 `<html lang="en">` → `<html lang="ko">`로 수정.
- `HomeClient.js`의 부제(`.subtitle`)를 `<p>`에서 `<h1>`로 변경 — 페이지 10개 중 홈만 유일하게 h1이
  없었던 문제 해결(태그만 바꿔서 디자인 변경 없음).
- `SiteHeader.js` 카테고리 드로어: 열리면 닫기 버튼으로 포커스 이동, `Esc`로 닫기, 닫히면 햄버거 버튼으로
  포커스 복귀, `role="dialog"` + `aria-modal="true"` 추가, 열려있는 동안 `document.body.style.overflow`로
  배경 스크롤 잠금. `document.body.style` 직접 대입이 `react-hooks/immutability` 린트 규칙에 걸려서
  `eslint-disable-next-line`으로 처리(모달 열림 중 스크롤 잠금은 표준 패턴이라 규칙 예외로 판단).
- 5개 퀴즈(`TetoEgenQuiz`/`HspQuiz`/`AttachmentQuiz`/`GamdasalQuiz`/`MbtiQuizClient`) 전부 동일 패턴 적용:
  문항 `<h1>`에 `ref` + `tabIndex={-1}`을 걸고, `step`이 바뀔 때마다 `useEffect`로 그 `h1`에 포커스를
  옮겨서 스크린리더/키보드 사용자가 "몇 번째 문항인지"를 놓치지 않게 함.
- `src/components/RelatedTests.js` 신규 — `testCatalog.js`에서 현재 테스트를 제외한 다른 테스트 1~2개를
  결과 화면 본문 안에 작게 추천(사이드레일과는 별개, 모바일에서도 항상 보임). 5개 퀴즈 결과 화면
  전부(기존 "홈으로" 링크 위)에 배치.
- 브라우저로 포커스 이동(`document.activeElement`로 문항 전환 시 h1 포커스 확인), 드로어 열기(`role=dialog`,
  `aria-modal`, 포커스, `body.style.overflow` 확인), `Esc` 닫기(포커스 복귀, 스크롤 해제 확인),
  `RelatedTests` 링크 노출까지 전부 실제 확인. 콘솔 에러 없음.

## 7주차 작업 (2026-07-11, Spec.md 2.5.10) — 앱 프레임 레이아웃 전환

사장님이 poomang을 다시 참고해서 확인한 결과, 6주차의 사이드레일 방식이 틀린 방향이었음 — 헤더만 화면
전체 너비로 늘어나고 본문만 좁아서 "PC 화면에 모바일 컴포넌트 하나 얹은 것"처럼 보이는 문제가 있었음.
올바른 방향은 헤더/본문/푸터 전체를 하나의 좁은 프레임으로 묶는 것.

- `src/app/layout.js`: `<body>` 안에 `SiteHeader`+`SiteShell`+`SiteFooter`를 통째로 감싸는 `.appFrame`
  컨테이너 추가(`globals.css`에 정의) — `max-width: 480px`, `margin: 0 auto`, `min-height: 100vh`,
  `background: var(--background)`, `flex: 1`(body가 이미 flex column이라 짧은 페이지에서도 푸터가
  아래로 밀리도록).
- `globals.css`: `body` 배경을 프레임 바깥 톤으로 분리 — 라이트 `#f2f2f2`, 다크 `#000000`(기존
  `--background: #0a0a0a`보다 한 톤 어둡게). 프레임 경계가 시각적으로 구분됨.
- `SiteShell.js`에서 `SideRail` import/사용 제거, 라우팅 분기를 "패턴 전용(`/`, `/tests/*`) vs 기본(그 외)"
  2분기로 단순화(6주차엔 레일 분기 포함 3분기였음). `SideRail.js` 파일 자체는 삭제하지 않고 남겨둠 —
  애드센스 승인 후 프레임 바깥 여백에 광고 자리로 재활용할 가능성(Spec.md 2.5.10 명시).
- `SiteShell.module.css`의 `.shell`을 레일 2단 레이아웃(`display:flex`+`justify-content:center`+
  `max-width:1200px`)에서 단일 컬럼(`flex-direction:column`, `flex:1`, `width:100%`)으로 정리, 미사용된
  `.content` 클래스 제거.
- `RelatedTests.js`는 지시대로 손대지 않고 그대로 유지.
- 검증: `npm run lint`/`npm run build` 통과(165 페이지 정적 생성 확인). 브라우저 1400px 뷰포트에서
  `getBoundingClientRect()`/`getComputedStyle()`로 직접 확인 — `.appFrame`이 헤더·본문·푸터를 전부 포함해서
  480px 폭으로 뷰포트 중앙에 위치(`frameLeft: 452.5, frameRight: 932.5`), 다크모드에서 body(`rgb(0,0,0)`)가
  frame(`rgb(10,10,10)`)보다 한 톤 어두움, 라이트모드에서 body(`rgb(242,242,242)`)/frame(흰색) 확인,
  `/mbti/estj`에서 `SideRail` 관련 요소 완전히 사라짐(`sideRailFound: false`) 확인. `/`, `/mbti/estj`,
  `/mbti/match/estj-infp`, `/tests/teto-egen` 전부 콘솔 에러 없이 정상 로드 확인.
  (환경 이슈로 스크린샷 캡처 자체는 실패했으나 — computed style/bounding rect 수치로 레이아웃을 직접
  검증했음, 코드 문제 아님.)

**7주차 사장님 검토 승인 완료 (2026-07-12)** — 앱 프레임 레이아웃 전환 확정.

## 8주차 작업 (2026-07-12, Spec.md 2.6) — 신규 테스트 4종

Spec.md에 신규 테스트 4종(신조어 트렌드 지수/밸런스게임/정신연령/커플 싱크로율)이 추가되어, 공수가 작은 순서대로
진행하고 커밋도 하나씩 나눠서 올렸다. 4개 모두 문항 카피는 초안이며 사장님 검토 전까지 확정 아님.

**① 정신연령 테스트 (2.6.3, 가장 단순)**
- 축(axis) 없이 옵션마다 매겨진 "나이 점수"의 평균을 내는 방식이라 `testEngine.js`를 그대로 쓸 수 없어서
  `src/lib/mentalAgeEngine.js` 신규(`calculateAverageAge`, `resolveAgeBracket`) — 문항/답변 완료 여부 체크는
  형태가 동일해서 `testEngine.js`의 `isComplete()`를 그대로 재사용함(중복 안 만듦).
- 8문항(주말 활동/스트레스 해소/콘텐츠 취향/소비 습관/음악/옷/프사/노화 체감), 옵션마다 10대(15)~40대+(48)
  대표 나이값 부여. 결과는 평균을 반올림해 "정신연령 OO세" + 구간별(10/20/30/40대+) 코멘트.
- 브라우저로 8문항 전부 같은 열(20대 옵션) 선택 → 평균 25 → "정신연령 25세 · 20대 감성" 정확히 계산됨 확인.

**② 밸런스게임 (2.6.2)**
- `testEngine.js`의 단일 축(즉흥형 vs 계획형)을 그대로 재사용하되, 결과 화면에 "내 선택 요약" 리스트를
  추가로 보여줘서 (질문별로 실제 고른 옵션 라벨) 결과 문구보다 이 리스트 자체가 콘텐츠가 되도록 함.
- 12쌍(짜장면/짬뽕, 계획형 여행/즉흥 여행, 아침형/저녁형, 카톡 칼답/읽씹 등) — 각 옵션에 즉흥/계획 성향을
  가볍게 배정(주관적 판단, 정답 개념 아님).
- 브라우저로 12문항 전부 첫 옵션 선택 → 결과 "계획형" + 12개 선택 요약 리스트가 실제 선택과 정확히 일치하는 것 확인.

**③ 신조어 트렌드 지수 (2.6.1, 시의성 유의)**
- 정답형 3지선다라 축 방식이 아니라 "맞춘 개수"로 채점해야 해서 `src/lib/quizEngine.js` 신규
  (`countCorrect`, `resolveScoreBand`) — `testEngine.js`/`mentalAgeEngine.js`와는 별개의 세 번째 채점 방식.
- Spec.md가 준 10문항(엠비스찬/커터칼퇴/장항준적 사고/슬프자베스/냐냐냥/못해솔로/셋로그/윤정아 챌린지/밈 확산
  경로/1.5가구 심리) 그대로 사용, 정답 위치는 매 문항 랜덤하게 섞어서 패턴으로 유추되지 않게 함. 결과 화면에
  문항별 정답/오답(O/X) + 정답 텍스트를 함께 보여주는 리뷰 리스트 추가.
- 결과 구간 0~3(아재 감성)/4~6(그럭저럭 트렌디)/7~9(찐트렌드피플)/10(밈 마스터), 놀리는 톤이 세지지 않게
  작성 시도했으나 **사장님이 한 번 더 톤 검토 필요** (2026년 상반기 유행어 기반이라 신선도가 생명이라는
  스펙 요구사항 반영, 유행 바뀌면 반기별 교체 검토는 Phase 3 이후 논의로 남겨둠).
- 브라우저로 매 문항 첫 옵션 선택 → 정답 위치가 0번인 문항 3개(q2, q5, q8)만 맞아 "3/10 · 아재 감성" 정확히
  나오는 것 확인, 리뷰 리스트의 O/X와 정답 텍스트도 실제 정답과 일치 확인.

**④ 커플 싱크로율 테스트 (2.6.4, 공수 가장 큼 — 아키텍처 신규)**
- DB/백엔드 없이 URL 인코딩만으로 두 사람 답변을 비교하는 구조 — Spec.md가 제시한 흐름 그대로 구현:
  1) A가 15문항 답변 → `src/lib/coupleSyncCode.js`의 `encodeAnswers()`가 답변 배열을 URL-safe base64
     문자열로 인코딩(`btoa` 기반, `+/=`를 `-_`로 치환)해서 `/tests/couple-sync/{code}` 링크 생성 →
     "이 링크를 상대방에게 보내세요" 화면(`CoupleSyncStart.js`).
  2) B가 그 링크(`/tests/couple-sync/[code]/page.js`, 동적 라우트)로 들어가면 `decodeAnswers()`가 A의
     답변을 복원하고, B에게 같은 15문항을 보여줌(`CoupleSyncJoin.js`).
  3) B가 다 답하면 클라이언트에서 바로 A/B 답변을 인덱스별로 비교해서 싱크로율(%) + 문항별 일치/불일치
     리스트를 렌더링. 서버/DB 저장 전혀 없음 — SSG 원칙 그대로 유지, `/tests/couple-sync/[code]`만
     `/result`·`/mbti/match/[pair]/opengraph-image`와 동일하게 `ƒ`(요청 시 서버 렌더링)로 빌드됨(무한한
     `code` 조합을 사전 생성할 수 없어서 당연한 결과 — 기존에도 있던 패턴이라 인프라 변경 없음).
  4) 문항 진행 UI(`CoupleSyncQuestions.js`)는 A/B 양쪽 플로우가 완전히 동일해서 공용 컴포넌트로 분리.
  5) `ShareButton.js`에 선택적 `url`/`label` prop 추가(기본값은 기존과 동일하게 `window.location.href`) —
     커플 싱크로율은 "현재 페이지"가 아니라 "생성된 코드 링크"를 공유해야 해서 필요했던 최소 확장,
     기존 5개 결과 화면 호출부는 prop을 안 넘기므로 동작 그대로 유지됨(회귀 없음, 브라우저로 재확인함).
  6) 링크가 깨졌거나 형식이 다르면(`decodeAnswers()`가 null 반환) "링크가 올바르지 않아요" + 새로 시작하기
     안내 화면으로 처리.
  15문항은 라이프스타일/가치관 위주(아침형·저녁형, 계획/즉흥, 정리정돈, 데이트 스타일, 갈등 해결, 소비
  습관, 반려동물, 여행 스타일 등)로 애착유형(연애 자체)과 겹치지 않게 구성.
- 브라우저로 전체 플로우 실제 확인: A가 15문항 응답 → 링크 생성(`.../tests/couple-sync/WzAs...`) →
  그 링크로 B 진입 → 앞 10문항은 A와 같은 선택, 뒤 5문항은 다르게 선택 → "67% · 15문항 중 10개 일치"로
  정확히 계산, 문항별 일치/불일치 표시도 실제 선택과 일치 확인. 깨진 코드로 진입 시 에러 화면도 확인.
  콘솔 에러 없음.

**공통**: 4개 테스트 모두 lint/build 통과 확인(최종 169페이지 정적 생성, `couple-sync/[code]`만 동적).
`testCatalog.js`에 4개 항목 추가(정신연령/밸런스게임=`personality`, 커플싱크로율=`emotion`, 신조어
트렌드=`personality`로 분류 — Spec.md에 카테고리 지정이 없어 기존 두 카테고리 중 가장 가까운 쪽으로 판단해서
배정함, 필요하면 조정 가능).

**8주차 사장님 검토 대기 중 — 문항 카피 전부 초안, 특히 신조어 트렌드 지수는 톤/시의성 재확인 요망.**
...
- **문항 카피 검토 대기**: 테토-에겐 q1~q12(초안), HSP 20문항 전체(초안), 애착유형 20문항 전체(초안),
  정신연령 8문항(초안), 밸런스게임 12쌍(초안), 신조어 트렌드 지수 10문항(초안, 톤/시의성 특히 재확인),
  커플 싱크로율 15문항(초안) — 확정 필요
- 로고(시바견) 파일 전달받으면 `SiteHeaderLogo.js`와 파비콘에 삽입 진행 — 그 전까지 헤더는 텍스트 워드마크만
- 테스트 카드 썸네일 이미지 진행 중 — MBTI 궁합만 시안 반영됨, 나머지 4개 카드 이미지와
  MBTI 궁합의 라이트 모드 버전 필요
- 신규 테스트(테토-에겐 등) 결과 화면에 사이드레일까지 적용할지는 Spec.md 2.5.9①에서 "추후 검토"로 남겨둠 —
  현재는 배경 패턴만 적용된 상태 그대로 유지
