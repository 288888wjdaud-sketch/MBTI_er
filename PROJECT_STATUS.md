...
## 브랜드/도메인 확정 (2026-07-10)
- 사이트명 "테스트할게"(기존 MBTIer에서 변경), 도메인 `willtest.kr` 가비아 구매 완료.
- **코드 반영은 아직 안 함** — 브랜드명 변경은 사장님이 다음 라운드에 별도 지시하기로 함(3주차 범위 아님).
  변경 시 `layout.js`의 metadata, `page.js`의 타이틀/부제, 각 테스트 페이지 metadata 등 "MBTIer" 텍스트가
  박힌 곳을 전부 찾아 교체 필요, `NEXT_PUBLIC_SITE_URL`도 `https://willtest.kr`로 교체 예정.
...
- **주의**: "감다살"/"감다뒤"라는 신조어의 정확한 의미를 확인하지 못해서 "감정이 살짝 예민해진 날/무뎌진 날"로
  해석해서 작성함. 실제 뉘앙스와 다르면 사장님이 확인 후 방향을 다시 잡아줘야 함.
- 브라우저로 5문항 완주 → 결과 표시까지 확인, 콘솔 에러 없음.

**② HSP 자애/타애 민감형 (`src/app/tests/hsp/`)**
...
- 브라우저로 8문항 완주(전부 첫 옵션 선택 → "내면 예민형" 결과) 확인, 콘솔 에러 없음.
...
- `GoogleAnalytics.js`와 같은 패턴: `NEXT_PUBLIC_ADSENSE_CLIENT_ID` 환경변수가 있으면 `adsbygoogle.js` 스크립트와
  ...
  브라우저로 환경변수 미설정 상태에서 placeholder가 그대로 나오는지 확인함.
...
- **문항 카피 검토 대기**: 테토-에겐 q1~q12(초안), 감다살/감다뒤 5문항, HSP 8문항 — 전부 미확정
- **"감다살/감다뒤" 신조어 의미 확인 필요** — 클로드 코드가 뜻을 추측해서 작성했음 (위 3주차 ① 참고)
- 브랜드명 "테스트할게"/도메인 `willtest.kr` 코드 반영 — 다음 라운드 지시 대기
- `public/eb9da986-....png`(원본 그리드, 2.5MB) 등 배포에 불필요한 파일 정리 검토 (Spec.md Phase 1-5)
- 4주차 예정: 성인 애착유형, 홈 화면 재설계(햄버거 메뉴, 카드 그리드) (Spec.md 참고)
