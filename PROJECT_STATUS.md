...
- `SiteShell`(client, 6주차 신규, `layout.js`에서 `{children}` 감쌈) — `usePathname()`으로 경로를 3분기
  (`/`·`/tests/*`는 배경 패턴만 / `/mbti/*`(유형·궁합 페이지)는 좌우 `SideRail` / 그 외는 기본)해서 적용.
  최초엔 홈도 레일 분기에 걸려 카드가 3중 중복되는 버그가 있었는데 6.1주차에 홈을 패턴 전용으로 재분류해 수정함.
...
- `RelatedTests`(server, 6.1주차 신규) — 퀴즈 결과 화면 본문 안에 다른 테스트 1~2개를 작게 추천(사이드레일과
  별개, 모바일에서도 항상 보임). 5개 퀴즈 결과 화면에 공용으로 사용.
